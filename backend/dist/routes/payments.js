...
import { capturePaypalOrder, createPaypalOrder, verifyPaypalWebhookSignature, } from "../services/payments/paypalService.js";
...
const paymentRateState = new Map();
const paymentRateLimit = (maxRequests) => {
    return (req, res, next) => {
...
const isValidCheckoutPayload = (payload) => {
...
const computeOrderFromDb = async (cartItems) => {
...
const hasProcessedMpesaCallback = (order, checkoutRequestId, status) => {
    return (order.paymentEvents || []).some((evt) => evt?.stage === "mpesa_callback" &&
        evt?.providerRef === checkoutRequestId &&
        evt?.status === status);
};
const hasProcessedPaypalWebhook = (order, eventId) => {
    return (order.paymentEvents || []).some((evt) => evt?.stage === "paypal_webhook" && evt?.providerRef === eventId);
};
// create orders
const createCheckoutOrder = async (payload, provider) => {
    return Order.create({
...
router.post("/paypal/create-order", async (req, res) => {
    try {
        const { total, name, address, email, phone, cartItems } = parseCheckoutPayload(req.body);
        const { pricedItems, total: serverTotal } = await computeOrderFromDb(cartItems);
...
router.post("/paypal/capture-order", async (req, res) => {
    try {
        ...
        const captureId = capture?.purchaseUnits?.[0]?.payments
            ?.captures?.[0]?.id;
        if (status === "COMPLETED") {
...
router.post("/mpesa/initiate", async (req, res) => {
    try {
        const { total, name, address, email, phone, cartItems, localOrderId } = parseCheckoutPayload(req.body);
...
router.post("/mpesa/callback", async (req, res) => {
    try {
        ...
        const providedToken = typeof req.query.token === "string" ? req.query.token : undefined;
        ...
        const resultDesc = callback?.ResultDesc || callback?.resultDesc || "Unknown result";
...
router.post("/paypal/webhook", async (req, res) => {
    try {
        ...
        const paypalOrderId = sanitizeText(req.body?.resource?.supplementary_data?.related_ids?.order_id, 80);
...
router.get("/orders/:orderId/status", async (req, res) => {
    try {
        ...
        const order = await Order.findById(req.params.orderId).select("_id paymentStatus orderStatus paymentProvider paymentRef totalPrice createdAt");
        if (!order)
            return res.status(404).json({ message: "Order not found" });
